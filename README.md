...
| File | Role |
|------|------|
| [`wordle_top.sv`](wordle_top.sv) | Top-level module that wires the ROM to the FSM. This is the IP packaged into the block design. |
...
| [`wordle_rom.sv`](wordle_rom.sv) | Synchronous 1024x32-bit ROM holding the dictionary (one 4-letter ASCII word per entry). |
...
| [`constraints.xdc`](constraints.xdc) | Timing constraint that defines the 100 MHz PL clock. |
...
- `o_result` `[7:0]`: 4 letters x 2-bit status; letter 0 in the most-significant pair (`[7:6]`)
...
`"skim"` becomes `0x736b696d`. The result is packed the same way, so reading a
result byte left-to-right matches the word left-to-right (and matches what the C
...
The two-cycle PROCESS to LATCH path guarantees the guess data is stable for a full
